		...
		if(kbhit() != 0) {
			...
			if(ch == 'q') {
				...
				for(int i=0;i<sizeof(gMissileObject)/sizeof(_S_BULLET_OBJECT);i++) {
					double vx,vy,c;
					for(int i=0;i<2;i++) {
						vx = gAlienObjects[i].m_fXpos - gPlayerObject[i].m_fXpos;
						vy = gAlienObjects[i].m_fYpos - gPlayerObject[i].m_fYpos;
					}
					c = sqrt(vx*vx+vy*vy);
					vx /= c;
					vy /= c;
					...
					if(pObj->m_nFSM == 0) { //슬립상태라면..
						for(int i=0;i<2;i++) {
							pObj->pfFire(pObj,gPlayerObject[i].m_fXpos,gPlayerObject[i].m_fYpos,5.0,vx,vy,10);
						}
						break;
					}
				}
